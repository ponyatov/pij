#include "hpp.hpp"							// === adaptation from Pascal prototype ===
...
Ez = (Fi[x][y]-Fi[x][y+1])*8000/0.001;		// Ez:=(Fi[x,y]-Fi[x,y+1])*8000/0.001;
Er = -1*(Fi[x][y]-Fi[x+1][y])*8000/0.001;	// Er:=-1*(Fi[x,y]-Fi[x+1,y])*8000/0.001;
z =z+Vz*dt+Ez*Qm*dt*dt/2;					// Z:=Z+Vz*dt+Ez*qm*dt*dt/2;
Vz =Vz+Ez*Qm*dt;							// Vz:=Vz+Ez*qm*dt;
R =R+Vr*dt+Er*Qm*dt*dt/2;					// R:=R+Vr*dt+Er*qm*dt*dt/2;
Vr =Vr+Er*Qm*dt;							// Vr:=Vr+Er*qm*dt;
t =t+dt;									// t:=t+dt;

te << z;									// write(te,z:8:6);
te << " ";									// write(te,' ');
te << r;									// writeln(te,r:8:6);
te << "\n";									// closefile(te);
...
int main (int argc, char *argv[]) {
	...
	assert (argc==4+1);	// pij.exe [V] [Qm] [Alpha] [r]
